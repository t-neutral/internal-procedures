...
**"Sincronizar" y "Añadir acceso directo a OneDrive": la diferencia.** Las dos traen la carpeta del equipo a tu ordenador y ninguna crea copias: las dos son reflejos del mismo original en SharePoint. Cambia dónde aparece y cómo se comporta. "Sincronizar" la coloca bajo el icono de **edificio** (`C:\Users\...\T_NEUTRAL\`), como una entrada separada. "Añadir acceso directo a OneDrive" la coloca **dentro de tu nube azul**, como una carpeta más de tu OneDrive.

Cada una tiene su ventaja, y la elección es una contrapartida real. "Sincronizar" mantiene la **separación visual** clara entre lo tuyo (nube azul) y lo del equipo (edificio), que es lo más fácil de tener claro cuando se empieza. El acceso directo es más **flexible y ligero** cuando manejas muchas bibliotecas o son grandes, pero mete lo del equipo dentro de tu OneDrive, difuminando esa frontera. La recomendación de T_NEUTRAL: **usa "Sincronizar"** mientras el equipo asienta la distinción entre lo personal y lo compartido; es la vía por defecto de la casa. Quien maneje muchas bibliotecas y tenga el mecanismo por la mano puede pasarse a accesos directos. Lo único que no se hace nunca es traer la misma biblioteca por las dos vías a la vez: **una biblioteca, una sola vía**, o acabarás con dos entradas al mismo sitio que confunden.
...
### 4.6. Por qué desde Teams no ves algunas carpetas: canal y carpeta

Al montar la estructura de un equipo aparece una confusión muy típica, y conviene entenderla porque si no, uno cree que algo se ha perdido. Dentro del sitio de un equipo, **cada canal es una carpeta**, y cuando entras a un canal desde Teams, la pestaña Archivos te mete **dentro** de esa carpeta, no en la raíz del sitio.

El caso concreto en que esto se nota: creas una carpeta en la raíz del sitio (desde SharePoint web o desde el Explorador), y luego, al entrar al canal General en Teams, no la ves. No se ha borrado; es que está en la raíz, un nivel por encima de donde Teams te coloca. Teams siempre te enseña el interior de un canal, nunca la raíz.

De esto sale una regla práctica al organizar un equipo: **lo que quieras trabajar y ver desde Teams tiene que estar dentro de la carpeta de un canal.** Si el equipo trabaja todo desde su canal General, tu estructura de carpetas (`01_`, `02_`…) va dentro de la carpeta `General`, no en la raíz. Si el equipo tiene varios canales, cada uno con su trabajo, la estructura va dentro de cada canal. La raíz del sitio, que solo se ve cómoda desde SharePoint web, se deja tranquila.

### 4.7. Dos entradas que parecen la misma: no son un duplicado

Otra sorpresa frecuente: al sincronizar un equipo, en el Explorador aparecen **dos entradas que contienen casi lo mismo**, con nombres como `Equipo - Documentos` y `Equipo - General`. Parece que se ha duplicado algo. No es así.

`Equipo - Documentos` es la **biblioteca completa** del sitio: la raíz, con todas las carpetas-canal dentro. `Equipo - General` es un **atajo a una sola carpeta**: la del canal General, que está *dentro* de la anterior. Es el mismo contenido visto por dos puertas, no dos copias. Lo que ves en `Equipo - General` está también, un nivel más abajo, dentro de `Equipo - Documentos`. (El nombre «Documentos» lo pone el sistema: es como se llama por defecto la biblioteca de cualquier sitio de SharePoint.)

Aparecen las dos cuando has sincronizado por las dos vías: el canal desde Teams (te da `- General`) y la biblioteca entera desde SharePoint web (te da `- Documentos`). Para quitarte la confusión, **quédate con una sola**:

- Si el equipo tiene o va a tener **varios canales** y quieres verlos todos juntos, quédate con la biblioteca completa (`- Documentos`) y deja de sincronizar el atajo al canal. (Aviso: los canales privados no aparecen dentro de esa biblioteca; cada uno es una biblioteca aparte y saldrá como su propia entrada. Eso es correcto, no un duplicado.)
- Si el equipo trabaja **todo desde el General**, quédate con el atajo al canal (`- General`) y deja de sincronizar la biblioteca completa.

La regla que evita que esto vuelva a pasar: **sincroniza cada equipo por una sola vía.** O los canales desde Teams, o la biblioteca desde SharePoint, no las dos.

### 4.8. Cómo quitar una sincronización sin borrar nada

Quitar una carpeta sincronizada **no se hace borrándola del Explorador.** Si la arrastras a la papelera directamente, puedes provocar sustos. La vía limpia es decirle a OneDrive que deje de sincronizarla, y él la retira solo, sin tocar nada de la nube.

**Paso 1.** Antes de nada, comprueba que no queda nada subiendo: que ningún archivo de esa biblioteca tenga las flechas azules girando. Con todo en círculo verde, está todo guardado en la nube.

**Paso 2.** Pulsa el icono de nube de OneDrive (abajo a la derecha, junto al reloj), luego el engranaje de **Configuración**, y entra en la pestaña **Cuenta**.

**Paso 3.** Busca en la lista la biblioteca que quieres quitar y pulsa **"Dejar de sincronizar"**. Confirma.

Al confirmar, OneDrive rompe el enlace y quita la carpeta del Explorador. Los archivos siguen intactos en SharePoint; solo desaparece la puerta local. Puedes volver a sincronizarla cuando quieras.

**Si después queda una carpeta vacía** con el nombre en el Explorador, es una carcasa sin contenido que a veces queda como resto. Esa sí la puedes borrar a mano, porque ya no está enlazada a nada. Si al borrarla dice que está en uso, cierra las ventanas del Explorador que la muestren y vuelve a intentarlo. (Para un **acceso directo** en vez de una sincronización, se quita distinto: clic derecho sobre la carpeta y "Eliminar", que borra solo el vínculo.)

### 4.9. No reorganices las carpetas sincronizadas en carpetas propias

Es tentador, cuando tienes varias carpetas de equipos bajo el edificio, querer agruparlas en una carpeta `Equipos` y otra `Interno` que crees tú. **No lo hagas.** Cada carpeta sincronizada es el punto de anclaje de un reflejo con la nube, no una carpeta corriente; si la metes dentro de otra, rompes la sincronización.

Lo que sí consigue el orden que buscas, sin romper nada: los nombres de los equipos. Si siguen una convención de prefijos (`INT - …`, `CLI - …`, `CIRTEXCAM - …`), en el Explorador salen agrupados solos por ese prefijo, que es el efecto de «los internos juntos, los de cliente juntos» sin carpetas contenedoras. Y si la lista se hace larga, la solución es dejar de sincronizar lo que no usas a diario, no apilarlo en carpetas.

La idea de fondo: **dentro de una biblioteca organizas las carpetas como quieras; por encima de las bibliotecas, el orden lo fija cómo están montados los equipos, y eso no se reorganiza desde el Explorador.**

### 4.10. Cómo funciona la sincronización mientras trabajas
...
**Cuando dos personas editan a la vez.** Aquí hay que distinguir dos situaciones que se comportan muy distinto. Si las dos tenéis el documento **abierto en Office al mismo tiempo**, funciona la coautoría: Office fusiona los cambios de ambas en el momento, veis los cursores de la otra, y no se pierde nada. Es lo normal y funciona bien con Word, Excel y PowerPoint. Pero si cada una edita **por su lado sin tenerlo abierto en común** (el caso típico: una trabaja sin conexión mientras la otra toca el mismo archivo en la nube), OneDrive no puede fusionar porque no vio los cambios ocurrir a la vez, y crea una **copia en conflicto**: guarda las dos versiones con nombres distintos («documento» y «documento - copia en conflicto de Nombre») y te avisa para que decidas cuál vale. No se pierde nada, pero toca reconciliar a mano. Se evita con lo de siempre: una sola copia viva, y si vais a editar a la vez, hacerlo con el documento abierto en común, no cada una por su lado. El trabajo sin conexión es seguro para lo que solo tocas tú; el riesgo aparece solo con documentos compartidos que otro puede estar editando en paralelo.

### 4.11. Las reglas de oro del trabajo en local
...
### 4.12. Cuándo conviene cada modo
...
**"Creé una carpeta en el sitio pero desde Teams no la veo."** Seguramente la creaste en la raíz del sitio, y Teams solo te muestra el interior de la carpeta del canal, no la raíz. Muévela dentro de la carpeta del canal (por ejemplo, dentro de `General`) y aparecerá. Como regla, lo que quieras ver desde Teams va dentro de un canal, no en la raíz (ver 4.6).

**"En el Explorador me aparecen dos carpetas del mismo equipo con casi lo mismo."** No es un duplicado: una es la biblioteca entera (`- Documentos`) y la otra un atajo al canal General (`- General`), que está dentro de la primera. Quédate con una y deja de sincronizar la otra (ver 4.7).

**"Quiero quitar una carpeta sincronizada que ya no uso."** No la borres del Explorador. Ve al icono de OneDrive, Configuración, pestaña Cuenta, y pulsa "Dejar de sincronizar" en esa biblioteca (ver 4.8). Los archivos siguen en la nube; solo desaparece de tu Explorador.
...
Tras leer esta guía, ya sabes cómo transcurre la vida diaria de un documento de Office: dónde nace según sea tuyo o del equipo, cómo se trabaja en línea o en local sin generar copias, cómo se comporta la sincronización y qué hacer cuando aparecen carpetas que parecen duplicadas o no se ven desde Teams, por qué no se renombra por versiones, por qué se comparte por enlace y no por adjunto, y cómo convocar reuniones y pasar archivos por Teams sin dejar copias sueltas.
